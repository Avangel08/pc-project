  ...
  return (
    ...
    >
      ...
      <div className="px-4 pt-2 pb-3">
        ...
        {/* Product Code */}
        {product.productCode && (
          <p className="text-xs text-muted-foreground mb-1">Mã: {product.productCode}</p>
        )}
        ...
        <div className="flex items-center justify-between">
          <span className="text-xs text-gray-400 font-medium bg-[#1A1A2E] px-2 py-1 rounded-full">{getStatusText()}</span>
          {product.quantity > 0 && product.quantity < 10 && (
            <span className="text-xs text-gray-400 font-medium bg-[#1A1A2E] px-2 py-1 rounded-full">Còn {product.quantity} sản phẩm</span>
          )}
        </div>
      </div>
    </div>
